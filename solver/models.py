...
class Figure(ABC):
    ...
    def _list_moves(self, jump_col: list, jump_row: list, linear: bool = False) -> set:
        moves = set()

        self.__check_array_lengths_equal(len(jump_col), len(jump_row))

        if linear:
            moves_in_line = self._max_moves
        else:
            moves_in_line = 1
...
class Knight(Figure):
    def list_available_moves(self) -> set:
        ...
        return self._list_moves(jump_col, jump_row)
...
class Bishop(Figure):
    def list_available_moves(self) -> set:
        jump_col = [1, 1, -1, -1]
        jump_row = [1, -1, 1, -1]
        return self._list_moves(jump_col, jump_row, linear=True)
